...
export default async function NetworkSettingsPage({
  ...
  const [config, sp, recentSubmissions] = await Promise.all([
    getConfig(),
    searchParams,
    db
      .select()
  ...
  async function saveNetworkSettings(formData: FormData) {
    ...
    await updateConfig({
      ...
      network: {
        ...
        // If the field was left blank (masked), keep the existing token; otherwise encrypt the new one.
        networkToken: rawToken ? encryptString(rawToken) : current.network.networkToken,
      },
    });

    revalidatePath("/admin/settings/network");
    redirect("/admin/settings/network?toast=saved");
  }

  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-bold">AEO Intelligence Network</h2>
      ...
      <form action={saveNetworkSettings} className="bg-white border border-zinc-200 rounded-lg p-6 space-y-5">
        {/* Participation toggle — rendered as a hidden input + visual toggle */}
        <div className="flex items-center justify-between">
          ...
          <label className="relative inline-flex items-center cursor-pointer ml-4 shrink-0">
            <input
              ...
              onChange={undefined}
            />
            {/* Hidden field carries the actual boolean value — JS updates it */}
      ...
      {/* Toggle sync script — updates the hidden field when the checkbox changes */}
      <script
...
`,
      ...
      <section className="bg-zinc-50 border rounded-lg p-4 text-sm text-zinc-500 space-y-1">
        ...
        <p className="text-xs mt-2">
          ...
          Requires the <strong>Bot Analytics</strong> plugin to be installed and active.
        </p>
      </section>
    </div>
  );
