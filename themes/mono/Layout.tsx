...
export default async function ThemeLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    ...
    >
      ...
      {isPreview && (
        <div className="fixed top-0 left-0 right-0 z-[9999] bg-amber-600 text-black text-xs font-medium text-center py-1.5 flex items-center justify-center gap-4" style={{ fontFamily: "var(--font-sans)" }}>
          <span>// preview: draft design changes</span>
          <a
